// text node type controller for run mode
(function(){
    if (!window.Sidebar) return;

    class TextNodeRunController {
        constructor(sidebar) {
        ...
        render(node) {
            ...
            // show sections for text nodes in run mode
            this.showSections([
                'execution_progress_group'
            ]);

            // hide sections not relevant for text nodes in run mode
            this.hideSections([
                'node_name',
                'python_file',
                'arguments_section',
    ...
    window.TextNodeRunController = TextNodeRunController;
})();
