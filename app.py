...
def browse_nodes():
    """list files and folders under the nodes directory for a given relative path."""
    ...
    try:
        ...
        # order everything alphabetically (mix folders and files)
...
@app.route('/api/nodes/mkdir', methods=['POST'])
def nodes_mkdir():
    """create a folder under nodes at the given relative path."""
    data = request.json or {}
...
@app.route('/api/nodes/touch', methods=['POST'])
def nodes_touch():
    """create a file under nodes at the given relative path.
    if the file name ends with .py, seed it with the contents of templates/template.py.
    """
    data = request.json or {}
    rel_path = (data.get('path') or '').strip().replace('\\', '/')
    name = (data.get('name') or '').strip()
    ...
    try:
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            if name.lower().endswith('.py'):
                # try to load python template content
                try:
                    # resolve template path relative to app root
                    template_path = os.path.abspath(os.path.join(os.path.dirname(__file__), 'templates', 'template.py'))
                    if os.path.exists(template_path):
                        with open(template_path, 'r', encoding='utf-8') as tf:
                            f.write(tf.read())
                    else:
                        # fallback minimal template if template file is missing
                        f.write('def my_function(argument1):\n\n    # put your script here \n\n    return argument1\n')
                except Exception:
                    # last-resort fallback to a minimal python file
                    f.write('def my_function(argument1):\n\n    # put your script here \n\n    return argument1\n')
            else:
                # non-python files start empty
                f.write('')
...
def nodes_move():
    """move a file or folder within the nodes directory.
    payload: { src: 'path/relative/to/nodes', dst_dir: 'target/folder/relative/to/nodes' }
    the file/folder will be moved under dst_dir using its basename.
    """
    data = request.json or {}
...
def nodes_delete():
    """delete a file or folder under nodes at the given relative path. if folder, delete recursively."""
    data = request.json or {}
    rel_path = (data.get('path') or '').strip().replace('\\', '/')
